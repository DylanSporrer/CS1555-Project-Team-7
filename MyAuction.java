...
public class MyAuction{
	...
	public static void custLogin(String validLogin, String validPass) throws SQLException{
		...
		do{
			...
			case 2:
				...
				//Query to return all with keyword(s) COPIED
				try{
			...
			case 3:
				...
				//Run proc_putProduct() with info COPIED
				try{	
			...
			case 4:
				...
				//Run proc_placeBid() with info COPIED
				try{
			...
			case 5:
				...
				int manageID = 0;
				//Run Query to show auctions by user COPIED
				try{
				...
				int sellPrice = 0;
				//Run Query to show sell price COPIED
				try{
				...
				if(ans.charAt(0) == 'y' || ans.charAt(0) == 'Y'){
					//Run Query to sell product with ID COPIED
					try{
					...
					//Run Query to withdraw auction with ID COPIED
					try{
					...
					//Run Query to Show Highest Bid COPIED
					try{
						PreparedStatement highestBid = dbcon.prepareStatement("SELECT MAX(amount) as highBid FROM BigLog WHERE aution_id = ?");
						highestBid.setInt(1, manageID);
						ResultSet resSet = highestBid.executeQuery();
						
						String highBid = resSet.getString(1);
	...
	public static void browse(String usr, String pass) throws SQLException{
		...
		//Query Shows Top Level of Category Tree COPIED
		try{
		...
		do{
			...
			case 1:
				...
				do{
					...
					//Query to display contents of category tree with currCategory as root COPIED
					try{
						//Run Query on Customer Table to check info
				...
				break;
			case 2:
				//Query to Browse by Price with Category = currCategory COPIED
				try{
				...
				break;
			case 3:
				//Query to Browse by Name with Category = currCategory COPIED
				try{
	...
	public static void adminLogin() throws SQLException{
		...
		do{
			...
			//Run Query on Admin Table to check info COPIED
			try{
		...
		do{
			...
			case 3:
				...
				do{
					...
					switch (selectTwo) {
					case 1:
						//Run Query to show all products COPIED
						try{
					...
					case 2:
						...
						//Run Query to show given a user COPIED
						try{
